...
categories = ["教程"]
tags = ["FFmpeg", "视频处理"]
title = 'FFmpeg 视频处理教程（八）：API、生态与流媒体'
+++

本篇从命令行实践继续向外扩展，介绍 FFmpeg 库层 API、常见工具链和 HLS、DASH 流媒体基础。
...
本文梳理 FFmpeg 的三种使用方式：CLI、脚本封装和 C API，并简要介绍 libavformat、libavcodec、libavfilter 各自负责什么，帮助你判断当前任务适合哪种方式、什么时候需要深入 C API。

### CLI 适合什么

FFmpeg CLI 适合绝大多数文件处理任务，例如转码、裁剪、拼接、压缩、加字幕、水印、切片和批处理。它的优势是经过长期验证、可组合，也容易被 shell、脚本和 AI agent 调用。
...
如果决定使用 API，可以先了解本文涉及的三个 C 库：libavformat 处理容器、libavcodec 处理编解码、libavfilter 处理滤镜。下面逐一说明各自负责的范围。

### libavformat

`libavformat` 负责 container 层处理，例如打开输入、读取 stream 信息、demux packet、写输出 container。它覆盖了 CLI 中打开文件、读取流信息、解封装和写输出容器等操作。
...
CLI 中的 `-vf`、`-af`、`-filter_complex` 都体现了 filter graph 的处理方式。

### 第一版不展开 C API 的原因

本教程第一版目标是建立视频处理任务能力，并让人和 AI 能稳定使用 FFmpeg。C API 学习成本高、代码量大、验证方式也不同于 CLI。过早展开会偏离这个目标。
...
FFmpeg 的音视频底层处理能力很强，但不是所有任务都适合只用 FFmpeg。实际工作中，经常会把 FFmpeg 和 OBS、HandBrake、Shaka Packager、GPAC、MediaInfo、Whisper、OCR 或视频理解模型组合使用。本文介绍这些工具的常见搭配方式和适用边界。

### OBS

OBS 适合录屏、直播推流、场景合成和实时采集。需要做直播、录制桌面或摄像头混流时，OBS 的图形界面和场景管理通常比手写 FFmpeg 命令更直观。

FFmpeg 更适合后处理：转码、裁剪、压缩、切片和批量处理。常见配合是 OBS 负责采集，FFmpeg 负责后续加工。

### HandBrake

HandBrake 是图形化转码工具，适合人工本地转码、压缩和格式转换。它的预设方案（preset）对入门用户更友好。

当任务需要脚本化、批处理、AI 调用或服务端运行时，FFmpeg 的命令行接口更合适。

### Shaka Packager

Shaka Packager 适合 HLS/DASH 打包、多码率输出和 DRM，面向专业的流媒体发布流程。FFmpeg 可以生成基础 HLS/DASH，但复杂发布工作流通常交给专用的打包工具。

### GPAC

GPAC 提供 `MP4Box` 等工具，适合 MP4 box 操作、DASH 打包和封装层检查。需要深入检查 MP4 结构时，它可以补充 FFmpeg/ffprobe 在封装结构检查上的视角。
...
- 媒体信息人工查看：MediaInfo。
- 字幕生成：Whisper + FFmpeg。
- 内容理解：视频理解模型/OCR + FFmpeg 抽帧。

## 流媒体和发布概览

本文覆盖面向 Web 和平台发布的几种常见输出方式：兼容 MP4、HLS、DASH，以及切片策略和上传前需要检查的项目。读完可以判断自己的场景该用哪种输出，以及每个命令的关键参数是什么。
...
如果场景需要分段传输，可以从 HLS 或 DASH 中选择。先看 HLS。
...
DASH 与 HLS 类似，同样采用 manifest 与 segment 组合。DASH 常见 manifest 是 `.mpd`。FFmpeg 能生成基础 DASH 输出，但更复杂的多码率、自适应和加密场景通常会使用 Shaka Packager 或 GPAC。
...
切片边界通常应落在 keyframe 上。若 keyframe 间隔太长，HLS/DASH segment 可能不稳定或不接近目标时长。常见做法是明确控制 GOP：
...
30 FPS 视频中 `-g 60` 大约是 2 秒一个 keyframe。实际项目还要结合平台规格、编码 ladder 和播放器行为来测试。

### 平台上传规格

不同平台对 codec、resolution、bitrate、frame rate、audio codec 和 loudness 可能有不同要求。处理前应先收集目标平台规格，不要凭经验猜测或直接让 AI 生成参数。
...
- subtitle：需要软字幕（soft subtitles）还是烧录字幕（burned subtitles）。
